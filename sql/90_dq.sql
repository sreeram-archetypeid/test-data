...
-- the 14 source CSVs, so a red row means the pipeline changed, never that the
...
checks AS (
  ...
  FROM UNNEST([
    STRUCT('DQ01' AS dq_id, 'fct_response row count'                        AS assertion, (SELECT COUNT(*) FROM f)                                             AS actual, 41770 AS expected),
    STRUCT('DQ02', 'distinct personas',                                     (SELECT COUNT(DISTINCT archetype_id) FROM f),                                                 398),
    STRUCT('DQ03', 'distinct question_key',                                 (SELECT COUNT(DISTINCT question_key) FROM f),                                                  95),
    ...
    STRUCT('DQ07b', 'imputed-age ROWS (fct grain) [F2]',                    (SELECT COUNT(*) FROM f JOIN a USING (archetype_id) WHERE a.age_band_is_imputed),            2390),
    ...
    STRUCT('DQ16a', 'dim_run row count',                                    (SELECT COUNT(*) FROM r),                                                                       14),
    STRUCT('DQ16b', 'dim_run sum of n_rows',                                (SELECT CAST(SUM(n_rows) AS INT64) FROM r),                                                   1790),
    ...
    STRUCT('DQ20e', 'metric_kind counts sum to the question total',         (SELECT COUNT(*) FROM q WHERE metric_kind IN
                                                                             ('ordinal_scale','categorical','multi_select','single_option','numeric_rating','open_end')),  95),
    STRUCT('DQ21', 'fct_response_option row count',                         (SELECT COUNT(*) FROM fo),                                                                  39888),
    ...
    STRUCT('DQ27b', 'questions classified categorical [F9]',                (SELECT COUNTIF(metric_kind = 'categorical') FROM q),                                           16),
    STRUCT('DQ28a', 'rows outside the QRE base [F11]',                      (SELECT COUNTIF(NOT is_in_qre_base) FROM f),                                                   676),
    STRUCT('DQ28b', 'rows inside the QRE base [F11]',                       (SELECT COUNTIF(is_in_qre_base) FROM f),                                                     41094),
    ...
    STRUCT('DQ29', 'fct_response row count unchanged by the base flag',     (SELECT COUNT(*) FROM f),                                                                   41770)
  ])
)
SELECT
